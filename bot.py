...
# 🍪 إعداد الكوكيز من متغير البيئة COOKIES_JSON
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        ...
        "• تجاوز حظر يوتيوب (كوكيز + محاكاة تطبيق)\n"
...
async def cmd_about(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    about_text = (
        ...
        "• تجاوز حظر يوتيوب عبر الكوكيز ومحاكاة التطبيق\n"
...
async def do_download(ctx, chat_id, url, fmt, qkey, status_msg, uid):
    ...
    # قائمة التنسيقات التي سنجربها إذا فشل الأول
    format_list = [fmt, "best", "bestvideo[height<=720]+bestaudio/best[height<=720]", "bestvideo[height<=480]+bestaudio/best[height<=480]", "worst"]

    filename = None
    for attempt, current_fmt in enumerate(format_list):
        if attempt > 0:
            await safe_edit(status_msg, f"🔄 إعادة المحاولة بصيغة مختلفة ({current_fmt})...")
        opts = {
        ...
        # ✨ محاكاة تطبيق أندرويد (الحل السحري الذي كان يعمل)
        if is_youtube:
            opts["user_agent"] = "com.google.android.youtube/19.29.36 (Linux; U; Android 14; en_US) gzip"
            opts["extractor_args"] = {"youtube": {"player_client": ["android", "ios", "web"], "skip": ["hls", "dash"]}}
