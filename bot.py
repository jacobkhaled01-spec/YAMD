...
# تثبيت aiohttp إذا لم يكن موجوداً
try:
...
# -------------------- PO Token Provider (بدون كوكيز) --------------------
try:
    import bgutil_ytdlp_pot_provider
except ImportError:
    subprocess.check_call([sys.executable, '-m', 'pip', 'install', 'bgutil-ytdlp-pot-provider'])
    import bgutil_ytdlp_pot_provider

import threading
def run_pot_server():
    from bgutil_ytdlp_pot_provider.server import app
    app.run(host='127.0.0.1', port=4416, threaded=True)
...
async def main():
    try:
        ...
        logger.info(f"PO Token Provider: ✅")
