...
def get_available_qualities(url: str):
    ...
    يحاول بعدة طرق قبل الاستسلام.
    """
    base = _ydl_base_opts(url)

    # محاولة 1: استخراج عادي
    try:
        with yt_dlp.YoutubeDL(base) as ydl:
            info = ydl.extract_info(url, download=False)
    except Exception as e1:
        # محاولة 2: بدون User-Agent مخصص + format=best
        try:
            opts2 = {
                "quiet": True, "no_warnings": True,
                "socket_timeout": 30, "no_check_certificate": True,
                "no_playlist": True, "format": "best",
            }
            if COOKIE_FILE and os.path.exists(COOKIE_FILE):
            ...
            # محاولة 3: extract_flat (أسرع، أقل تحققاً)
            try:
                opts3 = {
                    "quiet": True, "no_warnings": True,
                    "socket_timeout": 30, "no_check_certificate": True,
                ...
                if COOKIE_FILE and os.path.exists(COOKIE_FILE):
                    opts3["cookiefile"] = COOKIE_FILE
                with yt_dlp.YoutubeDL(opts3) as ydl:
                    info = ydl.extract_info(url, download=False)
            except Exception as e3:
                raise Exception(f"{e1}") from e3
    ...
    # إذا لم تُكشف أي جودات → أضف جودات افتراضية
    if not heights:
...
async def on_link(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        err = str(e)
        logger.warning(f"[on_link] فشل الفحص: {err[:100]}")
        # إذا فشل الفحص → عرض جودات افتراضية للمحاولة المباشرة
        if "format" in err.lower() or "sign in" in err.lower() or "unavailable" in err.lower():
            ctx.user_data["url"] = url
            kb = [[InlineKeyboardButton(f"🎥 {h}p", callback_data=f"q|h|{h}")]
                  for h in [480, 360, 240, 144]]
            kb.append([InlineKeyboardButton("🔊 MP3", callback_data="q|a|0")])
            await status.edit_text(
                "⚠️ تعذّر فحص الجودات تلقائياً.\nاختر الجودة وسيحاول البوت التحميل مباشرة:",
